...
M.deprecated_extras = {
  ["myvim.plugins.extras.formatting.conform"] = "`conform.nvim` is now the default **MyVim** formatter.",
  ["myvim.plugins.extras.linting.nvim-lint"] = "`nvim-lint` is now the default **MyVim** linter.",
  ["myvim.plugins.extras.ui.dashboard"] = "`dashboard.nvim` is now the default **MyVim** starter.",
}
...
function M.extra_idx(name)
  ...
  for i, extra in ipairs(Config.spec.modules) do
    if extra == "myvim.plugins.extras." .. name then
      return i
    end
  end
end
...
function M.fix_imports()
  Plugin.Spec.import = Util.inject.args(Plugin.Spec.import, function(_, spec)
    ...
    if dep then
      ...
      Util.warn(dep, { title = "MyVim", once = true, stacktrace = true, stacklevel = 6 })
      return false
    end
  end)
end
