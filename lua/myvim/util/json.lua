...
function M.migrate()
  Util.info("Migrating `myvim.json` to version `" .. Config.json.version .. "`")
  ...
  if not json.data.version then
    ...
    json.data.extras = vim.tbl_map(function(extra)
      return "myvim.plugins.extras." .. extra
    end, json.data.extras or {})
  elseif json.data.version == 1 then
    json.data.extras = vim.tbl_map(function(extra)
      -- replace double extras module name
      return extra:gsub("^myvim%.plugins%.extras%.myvim%.plugins%.extras%.", "myvim.plugins.extras.")
    end, json.data.extras or {})
  end
