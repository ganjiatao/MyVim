local Config = require("myvim.config")
...
---@class myvim.util.extras
local M = {}

---@type LazyExtraSource[]
M.sources = {
  { name = "MyVim", desc = "MyVim extras", module = "myvim.plugins.extras" },
  { name = "User", desc = "User extras", module = "plugins.extras" },
}

M.ns = vim.api.nvim_create_namespace("myvim.extras")
---@type string[]
M.state = nil
...
function X.new()
  ...
  self.float = Float.new({ title = "MyVim Extras" })
    ...
    if extra.row == pos[1] then
      ...
      Util.info(
        "`"
          ...
          .. "\nPlease restart MyVim to apply the changes.",
        { title = "LazyExtras" }
      )
      self:update()
      return
...
function X:render()
  self.text:nl():nl():append("MyVim Extras", "LazyH1"):nl():nl()
  self.text
    :append("This is a list of all enabled/disabled MyVim extras.", "LazyComment")
    :nl()
...
function X:extra(extra)
  ...
  if extra.source.name ~= "MyVim" then
